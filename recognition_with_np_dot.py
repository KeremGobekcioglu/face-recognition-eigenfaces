...
def load_olivetti_faces(root_dir, target_size=(100, 100)):
    image_paths = []
    labels = []
...
def compute_mean_face(dataset):
    ...
    mean_face_image = cv2.normalize(mean_face_image, None, 0, 255, cv2.NORM_MINMAX)
...
def compute_covariance_matrix(centered_data):
    ...
    return np.dot(centered_data, centered_data.T) / num_images
...
#     eigenfaces = np.dot(eigenvectors.T, centered_data)
...
def project_faces(centered_data, eigenfaces):
    ...
    return np.dot(centered_data, eigenfaces.T)
...
def recognize_face(test_face, mean_face, eigenfaces, projected_faces, labels, original_faces, fixed_threshold=300):
    ...
    test_face_image = cv2.normalize(test_face_image, None, 0, 255, cv2.NORM_MINMAX)
    mean_face_image = cv2.normalize(mean_face_image, None, 0, 255, cv2.NORM_MINMAX)
    closest_face = cv2.normalize(closest_face, None, 0, 255, cv2.NORM_MINMAX)
    reconstructed_face_image = cv2.normalize(reconstructed_face_image, None, 0, 255, cv2.NORM_MINMAX)
    ...
    test_face_image = cv2.resize(test_face_image, (200, 200))
    mean_face_image = cv2.resize(mean_face_image, (200, 200))
    closest_face = cv2.resize(closest_face, (200, 200))
    reconstructed_face_image = cv2.resize(reconstructed_face_image, (200, 200))
...
# Paths
olivetti_faces_dir = "real_dataset"  # Directory with Olivetti faces

# Step 1: Load and Resize Olivetti Faces
image_paths, labels = load_olivetti_faces(olivetti_faces_dir)
...
print("eigenfaces are calculated ")
# Create a subdirectory for this run's results
results_subdirectory = save_Results()

# Save mean face
mean_face_path = os.path.join(results_subdirectory, "mean_face.jpg")
cv2.imwrite(mean_face_path, mean_face_image_to_save)
...
for test_image_path, true_label in tqdm(zip(test_paths, test_labels), desc="Recognizing faces", total=len(test_paths)):
    ...
    recognized_label , combined_image = recognize_face(
        ...
        train_labels,
        train_dataset,  # Pass the true label to the function
